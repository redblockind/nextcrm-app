/**
 * MCP tools for CRM document operations.
 *
 * Upload/download URLs use Netlify Blobs via lib/storage.ts. The upload
 * endpoint is /api/upload/presigned-url (accepts FormData POST — the
 * "presigned-url" name is a legacy artifact from the MinIO era). Download
 * URLs are generated by storagePublicUrl() which returns /api/files/<key>.
 */
import { z } from "zod";
...
export const crmDocumentTools = [
  ...
  },
  {
    ...
    async handler(
      ...
      const uploadUrl = `/api/upload/presigned-url`;

      return itemResponse({ ...doc, uploadUrl, uploadMethod: "POST (FormData with file and folder fields)" });
    },
  },
  {
    ...
    async handler(args: { id: string }, userId: string) {
      ...
      return itemResponse({ id: doc.id, url: `/api/upload/presigned-url`, method: "POST (FormData)" });
    },
  },
  {
    ...
    async handler(args: { id: string }, userId: string) {
      ...
      if (!doc) notFound("Document");
      if (!doc.key) validationError("Document has no storage key");
      const url = storagePublicUrl(doc.key!);
      return itemResponse({ id: doc.id, url });
    },
  },
  {
